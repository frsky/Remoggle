...
const repeatLines = ['Would you like another new word?', 'Do you want to go again?', 'Shall I create another new word?', 'Shall I invent another new word?', 'Do you want another new word?', 'Do you want a fresh word?'];
...
const handlers = {  // Handle initial stateless intents
    'LaunchRequest': function() {
        ...
        const speechOutput = "Word Maker creates new nonsense words. " + varyIntent[Math.floor(Math.random() * varyIntent.length)];
        const reprompt = varyIntent[Math.floor(Math.random() * varyIntent.length)];
...
const wordHandler = Alexa.CreateStateHandler("WORDMODE", {
    'LaunchRequest': function() {
        this.handler.state = "ASKMODE";
        const speechOutput = "Word Maker creates made-up English words. " + varyIntent[Math.floor(Math.random() * varyIntent.length)];
        const reprompt = varyIntent[Math.floor(Math.random() * varyIntent.length)];
        this.response.speak(speechOutput).listen(reprompt);
        this.emit(':responseReady');
    },
    ...
    'doMakeAWord': function() {
        ...
        console.log(spelled);
        this.response.speak
        const wordOutput = varyDeliver + ' ' + word + '. <p>' + spelled + '.</p> ' + word + '. ' + moment + repeatLines[Math.floor(Math.random() * repeatLines.length)];
        this.response.speak(wordOutput).listen(repeatLines[Math.floor(Math.random() * repeatLines.length)]);
        this.emit(':responseReady');
    ...
    'AMAZON.NoIntent': function() {
        ...
        'Unhandled': function() {
        this.handler.state = "ASKMODE";
        const speechOutput = "I didn't get that. " + varyIntent[Math.floor(Math.random() * varyIntent.length)];
        const reprompt = varyIntent[Math.floor(Math.random() * varyIntent.length)];
        this.response.speak(speechOutput).listen(reprompt);
        this.emit(':responseReady');
    },
